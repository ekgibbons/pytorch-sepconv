...
__global__ 
void kernel_SeparableConvolution_updateOutput(
       	const int n,
	...
	float* output, const long4 output_size, const long4 output_stride) 
{
	...
	for (int intFilterY = 0; intFilterY < FILT_LENGTH; intFilterY += 1) 
	{
	    for (int intFilterX = 0; intFilterX < FILT_LENGTH; intFilterX += 1) 
	    {
		float product =  IDX_4(input, intBatch, intDepth, intY + intFilterY, intX + intFilterX) * 
...
/****************************************************************************************/
/*                                 Backwards Vertical Grad                              */ 
/****************************************************************************************/

__global__ 
void kernel_SeparableConvolution_updateGradVertical(
    const int n,
    ...
    const long4 gradVertical_stride)
{
	...
	for (int intFilterX = 0; intFilterX < FILT_LENGTH; intFilterX++) 
	{
	    float product = IDX_4(gradLoss, intBatch, 0, intY, intX)*              // channel 0
	...
	float* gradHorizontal, const long4 gradHorizontal_size, const long4 gradHorizontal_stride)
{
	...
	for (int intFilterY = 0; intFilterY < FILT_LENGTH; intFilterY++)
	{
	    float product = IDX_4(gradLoss, intBatch, 0, intY, intX)*             // channel 0
		IDX_4(input, intBatch, 0, intY + intFilterY, intX + intFilterX)* 
		IDX_4(vertical, intBatch, intFilterY, intY, intX) + 
